...
@pytest.mark.parametrize(
    "dtype", [torch.float32])  # NOTE that float16 will cause problem
@pytest.mark.parametrize("id_dtype", [torch.int64, torch.int32])
def test_fused_softmax_argsort(m, k, seed, descend, dtype, id_dtype):
    torch.manual_seed(seed)
    ...
    fused_softmax_sort_kerenl[grid](x, o, ids, x.stride(0), x.stride(1),
                                    int(descend), BLOCK_M, BLOCK_N, STABLE)
