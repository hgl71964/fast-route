...
def run_group(
    ...
    topk_weight, topk_ids = torch.topk(score, topk)
    _ = fused_moe_grouped(a, w1, w2, topk_weight, topk_ids, False)
...
benchmark.run(show_plots=True, print_data=True, save_path=os.path.dirname(os.path.abspath(__file__)))
...
# run_group(m, n, k, e, topk, torch.float16)