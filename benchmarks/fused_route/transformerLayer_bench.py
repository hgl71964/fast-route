...
if __name__ == '__main__':
    ...
    def benchmark(seq, provider, n, k, e, topk):
        ...
        x = torch.randn((args.b, seq, args.k), device='cuda', dtype=dtype) / 10
        ...
        if provider == 'vllm':
            ms, min_ms, max_ms = triton.testing.do_bench(
                ...
                quantiles=quantiles)
        perf = lambda ms: (2 * args.b * seq * n * k + 2*args.b*seq*seq*args.n_head*args.head_dim) * 1e-12 / (ms * 1e-3)
        return perf(ms), perf(max_ms), perf(min_ms)

    benchmark.run(show_plots=True,
                  print_data=True,
                  save_path=f'./data/transformer_bench_{n}_{k}_{e}_{topk}')
